...
开启时联动控制模块 ``lock_layout``（布局锁定 + 全部窗口鼠标穿透），并经
``locked_windows`` 记录当下双击锁定的窗口（只有这些窗口能响应窗口快捷键）；
关闭时 ``unlock_layout`` 还原。模式开启后才可使用全局热键：主开关（默认
...
class HotkeyMode(QAbstractNativeEventFilter):
    ...
    def set_enabled(self, enable: bool) -> None:
        ...
        if enable:
            # 控制模块：锁定布局 + 记录当下双击锁定的窗口。此后窗口因鼠标
            # 穿透不可操作，双击锁定状态与记录表在模式存续期间保持一致。
