"""翻译历史浏览窗口：常规窗口阅读 ``translations.db`` 的分桶翻译记录。

布局与交互（需求给定 .raw_plans/future_翻译历史窗口.txt）：上下两部分 ——
上部为「查找」标签 + 文本输入框 + 上一个/下一个/删除按钮；下部为
QTableView + QAbstractTableModel 的两列表格（原文/译文），按写入顺序滚动
浏览（外观如两列的 Excel 表格）。

分桶存储的处理（需求强调）：translations.db 以原文哈希为键、
``(hash, original)`` 为复合主键，同一哈希下可共存多条不同原文的记录；
浏览时逐行展开（每行即一组原文译文，rowid 序 = 写入顺序）；删除仅移除
选中的那一组 ``(hash, original)`` 记录，同桶内其它原文的记录不受影响，
桶内无剩余记录时整个条目随之消失（语义由 :meth:`TranslationStore.delete`
按复合主键精确删除天然保证）。
...
logger = logging.getLogger("renpy_overlay.translation_history_window")
...
class _TranslationTableModel(QAbstractTableModel):
    """翻译记录两列表格模型：原文/译文。

    行缓存与 ``TranslationStore.entries()`` 同构：(hash, original, translated)
    —— hash 不参与展示，仅作为删除时定位分桶记录的依据；匹配当前查询词的
    行经 BackgroundRole 整行高亮。
    """

    def __init__(self, store: TranslationStore | None) -> None:
        super().__init__()
        self._store = store
        #: 行缓存与 TranslationStore.entries() 同构：(hash, original, translated)
    ...
    def set_records(self, records: list[tuple[str, str, str]]) -> None:
    ...
    def original_at(self, row: int) -> str | None:
        """第 ``row`` 行的原文（删除按复合主键 hash+original 定位）；越界返回 None。"""
...
class _CopyTableView(QTableView):
    ...
    def _copy_selection(self) -> None:
        ...
        if lines:
            ...
            logger.debug("已复制翻译记录文本（%d 行）", len(lines))


def _dark_palette() -> QPalette:
    """翻译历史窗口的暗色调色板（色彩方案参照识曲历史窗口：深底浅字 + 蓝强调）。"""
    palette = QPalette()
...
class TranslationHistoryWindow(QWidget):
    """翻译历史浏览窗口（单例复用：close = hide，refresh 重建数据）。"""
    ...
    def __init__(self, store: TranslationStore | None) -> None:
        ...
        self._model = _TranslationTableModel(store)
    ...
    def _find(self, forward: bool) -> None:
        ...
        if index == -1:
            logger.info("翻译历史查找无匹配（query=%r）", query)
            return
    ...
    def _delete_selected(self) -> None:
        """删除当前选中行的一组翻译记录并同步数据库，随后重建表格并原位回选。

        分桶语义（需求）：仅删除选中的这一组 ``(hash, original)``；同桶内
        其它原文的记录保留，桶空则整个条目随之消失（由 store 按复合主键
        精确删除天然保证）。
        """
        row = self._table.currentIndex().row()
        if row < 0:
            logger.info("没有选中的翻译记录，忽略删除请求")
            return
        original = self._model.original_at(row)
        if not original:
            return
        if self._store is None or not self._store.delete(original):
            logger.warning("删除翻译记录失败（原文 %d 字）", len(original))
            return
        logger.info("已删除翻译记录（原文 %d 字）", len(original))
        self.refresh()
