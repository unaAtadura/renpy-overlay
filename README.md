...
工具端自己的进程里用 PyQt6 创建**全透明悬浮窗**（标题 + 正文，注入成功后另增
一个绑定窗口，见下），各窗均带
...
- **截图翻译**：标题/正文窗文字处**右键**弹快捷菜单 —— 创建截图窗口（最多 8 个，
  ...
  并提示「翻译失败」；
- **锁鼠标区域**：快捷菜单「锁鼠标区域: 关/开」（默认关，位于快捷键模式之下）
  —— 窗口化运行游戏时把鼠标活动范围限制在框选区域内，防止鼠标点出游戏窗口。
  开启后出现灰色边框（#808080）的范围窗口，可拖动、八向拉伸；在窗口内**双击**
...
- **快捷键模式**：快捷菜单开关（默认关）。开启后整体隐藏全部截图窗口并锁定布局
  ...
  `ctrl+alt+o`）在 Ren'Py 游戏内触发后即使全部松开，游戏仍持续快进（Ctrl 是
  Ren'Py 的快进键）——与是否运行/注入本工具无关，属 Ren'Py 输入栈自身行为，
  程序侧补发 key-up 无法治愈；开启快捷键模式时若主开关键含 Ctrl 会提示规避，
  建议在 config.json 更换为不含 Ctrl 的组合（如 `alt+f9`）；
- **遥控截图**：快捷键模式的附属功能（配置 `remote_screenshot_enable`，默认开启）——
  进入快捷键模式时生成两个可独立拖动的小圆形悬浮窗：窗口 A（触发器，无边框纯色
  半透明粉色圆 `#F7A8B8`）与窗口 B（瞄准器，同色边框 + 黑色半透明蒙版的圆形），
  直径分别由 `remote_screenshot_diameterA` / `remote_screenshot_diameterB` 控制
  （默认 100，最小 20）；退出快捷键模式即销毁。把窗口 B 拖到**已双击锁定**的截图
  区域上，B 变为正方形（边长 = 直径、中心点重合，几何尺寸不变）且边框色变为该
  截图区域窗口的颜色，即“装填”完成；拖到未锁定区域不变形并忽略触发；与多个锁定
  区域相交时第一个进入的驻留生效，离开后恢复圆形。此时单击窗口 A 即对装填的区域
  发起截图翻译（复用既有截图翻译入口：同一时刻只允许一个在途 API、提示规则一致，
  截图期间 A/B 与其它悬浮窗一并隐藏、结束后恢复）。该功能与快捷键模式主开关相互
  独立——主开关关闭时遥控截图照常工作；
...
- **翻译历史**：快捷菜单「查看历史 → 翻译历史」—— 常规窗口按写入顺序浏览
  ...
  随之消失；译文列双击（或选中后按 F2）进入行内编辑，回车写入数据库
  （空译文不写入、恢复显示原译文；修改不立即上屏，配合「定位翻译位置」
  清空内存缓存后下一次命中即取新译文）；窗口打开/刷新可携带 (哈希, 原文)
  定位入参，直接滚动并选中指定条目；
- **标题窗入口条：预构建翻译缓存 / 定位翻译位置**：注入成功（Hook 层确认）
  后，标题窗标题文本右侧出现自绘图标入口条（跳过注入模式不出现；可脱离
  注入运行的功能仍在右键快捷菜单）。入口「预构建翻译缓存」：点击后扫描游戏
  目录 `game/` 下的 `.rpy`（含 `.rpa` 归档内）相对路径，弹窗勾选本轮要构建的
  文件（默认不勾选，支持分批）→「解析」后先统计缺失条目数，再后台逐条翻译
  并写入 `renpy_overlay_cache/translations.db`（只补缺失；清洗口径与运行时
  完全一致，运行时自动翻译两级缓存直接命中预填条目）。预构建不受「同时仅
  一个 API 请求」互斥约束 —— 后台翻译期间单击/自动翻译照常；写库经进程级
  写锁与前台串行化；「隐藏到后台」后再次点击入口唤回，弹窗内可取消；游戏
  退出自动停止且不留残缺条目，下次进入自动跳过已入库条目；入口「定位翻译
  位置」：打开翻译历史窗口（已打开则刷新置前）并定位最近一次缓存命中的
  记录（按哈希 + 原文滚动选中），随后立即清空内存缓存——人工在历史窗口
  改译后，下一次自动翻译命中即回源数据库取到最新译文，无需重启工具；
...
| 右键（标题/正文文字处） | 快捷菜单：创建/销毁截图窗口、快捷键模式、锁鼠标区域、听歌识曲、对话、查看历史（翻译历史/截图历史/识曲历史/对话历史）、退出 |
| 截图窗双击 | 锁定/解锁截图窗口（锁定后单击即截图翻译） |
| 遥控截图窗 A：左键单击（快捷键模式内） | 对窗口 B 已“装填”的截图区域发起截图翻译（未装填则忽略）；按住拖动可移动窗口 |
| 遥控截图窗 B：按住拖动 | 移动瞄准窗口：拖入已双击锁定的截图区域变正方形（边框色 = 区域色）即装填，离开恢复圆形 |
...
- **自动翻译**：项目根目录的 `config.json`（首次运行自动创建、已被 `.gitignore`
  ...
  ```json
  {
    ...
    "remote_screenshot_enable": true,  // 遥控截图开关（快捷键模式期间生成 A/B 小圆窗）
    "remote_screenshot_diameterA": 100, // 窗口 A（触发器）直径（像素，最小 20）
    "remote_screenshot_diameterB": 100, // 窗口 B（瞄准器）直径（像素，最小 20）
...
- `tests/test_screenshot_vision.py`：vision 请求体（image_url base64、思考模式方言）、
  译文解析与错误路径（本地 HTTP 桩）；
- `tests/test_mouse_lock.py`：锁鼠标区域状态机（开关联动热键注册、双击锁定/
  逃脱热键解锁、ClipCursor 施加与解除、提示文案，全部 stub 注入离线验证）；
...
│  │  ├─ mouse_lock_window.py  # 锁鼠标区域范围框选窗口（灰边框、八向拉伸、双击锁定）
│  │  ├─ mouse_lock_indicator.py # 锁鼠标区域锁定态提示窗口（等大小、鼠标穿透）
│  │  ├─ mouse_lock.py         # 锁鼠标区域控制器（ClipCursor 限制 + 逃脱热键）
...
│  ├─ translation_history_window.py # 翻译历史浏览窗口（分桶展开两列表格：查找/删除/复制/译文编辑/定位）
